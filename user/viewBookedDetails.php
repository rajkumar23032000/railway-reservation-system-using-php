<?php 
    ...
    if(isset($_SESSION["email"])){
        ...
        $result3 = mysqli_query($mysqli, "SELECT * FROM booking_details WHERE train_no='$trainNo' and email_id='$email' and booked_time='$bookedTime'");
        if(!$result3){
            die("Failed to fetch booking details from DB : " . mysqli_error($mysqli));
        }
...
<?php 
    if(isset($_POST['cancel-tickets-btn'])){
        sleep(5);
        $nos = (int)$res3['no_of_seats'];
        $update_tickets_booked_query = mysqli_query($mysqli, "UPDATE train_details SET tickets_booked=$tb-$nos WHERE train_no='$trainNo'");
        if(!$update_tickets_booked_query){
            die("Failed to update tickets booked in train details in DB : " . mysqli_error($mysqli));

        }

        $delete_ticket_query = mysqli_query($mysqli, "DELETE FROM booking_details WHERE train_no='$trainNo' and email_id='$email' and booked_time='$bookedTime'");
        if(!$delete_ticket_query){
            die("Failed to delete tickets from booking details in DB : " . mysqli_error($mysqli));
        }
    ...
    <body>
        ...
        <div class="container view-book-cont">
            <?php 
                ...
                echo "<br><br>";
                echo "<i class='far fa-clock'>  ".$res['journey_timing']."</i><span class='text-muted'>  (Starting Time)</span>";
                echo "<br><br>";
                ...
                echo "<br><br>";
                echo "<i class='fas fa-tags badge badge-danger price-tag-1'>  $".$res['price']."</i><span class='text-muted'>  (per ticket)</span>";
                echo "<br><br>";
            ?> 
            ...
            <form action="" method="post" class="book-seats-form">
                <div class="form-group">
                    <?php
                        echo "<i class='fas fa-tags badge badge-primary price-tag-1'>  $".($res['price'] * $res3['no_of_seats'])."</i><span class='text-muted'>  (Total Cost)</span>";
                        echo "<br><br>";
                        echo "<i class='fas fa-chair badge badge-secondary no-of-seats-icon'>  ".$res3['no_of_seats']."</i><span class='text-muted'>  (No.of Seats Booked)</span>";
                    ?>
                    <br>
                    <button id="cancel-tickets-btn" name="cancel-tickets-btn" class="btn btn-warning btn-lg font-weight-bold my-3" onclick="cancelledSuccess()">Cancel Seats</button>
                    <h3 id="cancelled-success" class="text-success font-weight-bold">Cancelled Successfully</h3>
                </div>
            </form>
        </div>
